 ...
 */
public class VmAllocationPolicyTA extends VmAllocationPolicy {
	 ...
	 * @post $none
	 */
	public VmAllocationPolicyTA(List<? extends Host> list) {
		super(list);
	...
	public boolean allocateHostForVm(Vm vm) {
		...
		if (!getVmTable().containsKey(vm.getUid())) { // if this vm was not created
			do {// we still trying until we find a host or until we try all of them
				Log.printLine("start - try to allocate vm " + String.valueOf(vm.getId()) + " " + String.valueOf(freePesTmp.size()));
				int idx = -1;

				// we want the host same id as vm
				for (int i = 0; i < freePesTmp.size(); i++) {
					if (i == vm.getId()) {
						idx = i;
						Log.printLine("VM" + "ALLOCATED" + String.valueOf(idx) + " HOST" + String.valueOf(i));
						break;
					}
				}
				Log.printLine("try to allocate vm to" + String.valueOf(idx));

				Host host = getHostList().get(idx);
				result = host.vmCreate(vm);

				if (result) { // if vm were succesfully created in the host
					Log.printLine("success alocate" + String.valueOf(idx));
					...
					Log.printLine("FAILED TO CREATE VM");
				}
				tries++;
