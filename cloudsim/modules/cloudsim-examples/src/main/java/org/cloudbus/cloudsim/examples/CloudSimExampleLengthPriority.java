 ...
 */
public class CloudSimExampleLengthPriority {
	...
	public static void schedule(DatacenterBroker broker, List<Cloudlet> cloudlets, List<Vm> vms) {
		Comparator<Cloudlet> sortByLength = new Comparator<Cloudlet>() {
			...
			public int compare(Cloudlet cloudlet1, Cloudlet cloudlet2) {
				return (int) (cloudlet1.getCloudletLength() - cloudlet2.getCloudletLength());
			}
		};
		...
		for(int i= 0; i < cloudlets.size(); i++) {
			...
			// Log.printLine(cloudlet.getCloudletLength());
			cloudlet.setClassType(i+1);
			Log.printLine("Task type" + cloudlet.getClassType());
			vmIndex++;
			if(vmIndex > maxIndex) {
				vmIndex = 0;
			}
	...
	private static void printCloudletList(List<Cloudlet> list) {
		...
		Log.printLine("Total penalty in seconds " + penalty);
